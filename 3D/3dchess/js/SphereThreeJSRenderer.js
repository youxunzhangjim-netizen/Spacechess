...
export class SphereThreeJSRenderer extends TorusThreeJSRenderer {
    ...
    createBoard3D() {
        ...
        const base = new THREE.Mesh(
            new THREE.SphereGeometry(this.sphereRadius - 0.035, 112, 56),
            this.materials.base
        );
        base.scale.y = this.verticalScale;
        base.castShadow = true;
        base.receiveShadow = true;
        base.userData = { type: 'sphere-base' };
    ...
    addWindingGuides() {
        // Sphere mode has no glued seam arrows; the longitude wrap is shown by the closed latitude rings.
    }
    ...
    createLongitudeLine(xBoundary, material) {
        ...
        const phiStart = (this.playableMinY() / this.boardHeight()) * Math.PI;
        const phiEnd = ((this.playableMaxY() + 1) / this.boardHeight()) * Math.PI;
        const segments = 96;

        for (let i = 0; i <= segments; i++) {
            const phi = THREE.MathUtils.lerp(phiStart, phiEnd, i / segments);
            points.push(this.pointOnSphere(theta, phi, this.surfaceLift + 0.035));
        }
    ...
    createLatitudeLine(yBoundary, material) {
        const points = [];
        const phi = (yBoundary / this.boardHeight()) * Math.PI;
        const segments = 144;
    ...
    createCellGeometry(x, y, lift = 0, inset = 0.018) {
        ...
        const phi0 = ((y + inset) / this.boardHeight()) * Math.PI;
        const phi1 = ((y + 1 - inset) / this.boardHeight()) * Math.PI;
        ...
        for (let iy = 0; iy <= phiSegments; iy++) {
            const phi = THREE.MathUtils.lerp(phi0, phi1, iy / phiSegments);
            for (let ix = 0; ix <= thetaSegments; ix++) {
                const theta = THREE.MathUtils.lerp(theta0, theta1, ix / thetaSegments);
                const point = this.pointOnSphere(theta, phi, this.surfaceLift + lift);
    ...
    getCellPose(x, y, lift = 0) {
        ...
        const phi = ((y + 0.5) / this.boardHeight()) * Math.PI;
        const normal = this.normalOnSphere(theta, phi);
        const position = this.pointOnSphere(theta, phi, this.surfaceLift + lift);
        const tangentU = new THREE.Vector3(-Math.sin(theta), 0, Math.cos(theta)).normalize();
        const tangentV = new THREE.Vector3(
            Math.cos(phi) * Math.cos(theta),
            -Math.sin(phi) * this.verticalScale,
    ...
    pointOnSphere(theta, phi, lift = 0) {
        const radius = this.sphereRadius + lift;
        const sinPhi = Math.sin(phi);
        return new THREE.Vector3(
            radius * sinPhi * Math.cos(theta),
            radius * Math.cos(phi) * this.verticalScale,
            radius * sinPhi * Math.sin(theta)
        );
    }

    normalOnSphere(theta, phi) {
        const sinPhi = Math.sin(phi);
        return new THREE.Vector3(
            sinPhi * Math.cos(theta),
            Math.cos(phi) / this.verticalScale,
